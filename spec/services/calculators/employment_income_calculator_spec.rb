...
module Calculators
  RSpec.describe EmploymentIncomeCalculator, :vcr do
    ...
    let(:first_employment) { OpenStruct.new(entitles_employment_allowance?: entitles_employment_allowance) }
    ...
    let(:second_month) { Date.parse("2021-04-30") }
    let(:third_month) { Date.parse("2021-05-30") }
    let(:fourth_month) { Date.parse("2021-06-30") }
    let(:dates) { [second_month, third_month, fourth_month] }
      ...
      context "at least one employment record exists" do
        it "adds the fixed employment allowance from the threshold files" do
          expect(described_class.call(submission_date: assessment.submission_date,
                                      employment: first_employment).result.fixed_employment_allowance).to eq(-45)
        end
        ...
        context "if applicant is not in work" do
          ...
          it "ignores fixed employment allowance" do
            expect(described_class.call(submission_date: assessment.submission_date,
                                        employment: first_employment).result.fixed_employment_allowance).to eq(0)
          end
        end
      end
    end
