module Creators
  class OtherIncomesCreator
    ...
    class << self
      def call(gross_income_summary:, other_incomes_params:)
        ActiveRecord::Base.transaction do
          create_other_income(gross_income_summary:, other_incomes_params:)
        end
    ...
    private
      ...
      def create_other_income(gross_income_summary:, other_incomes_params:)
        other_incomes(other_incomes_params).each do |other_income_source_params|
          create_other_income_source(gross_income_summary:, other_income_source_params:)
        end
      end

      def create_other_income_source(gross_income_summary:, other_income_source_params:)
