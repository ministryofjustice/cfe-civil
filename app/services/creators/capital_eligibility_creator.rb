  ...
  class CapitalEligibilityCreator
    class << self
      def call(proceeding_types:, level_of_help:, submission_date:, assessed_capital:)
        proceeding_types.map { |proceeding_type| assessed_eligibility(proceeding_type:, level_of_help:, submission_date:, assessed_capital:) }
      end

      def unassessed(proceeding_types:, level_of_help:, submission_date:)
        proceeding_types.map { |proceeding_type| create_unassessed(proceeding_type:, level_of_help:, submission_date:) }
      end

    private

      def assessed_eligibility(proceeding_type:, level_of_help:, submission_date:, assessed_capital:)
        create_eligibility(proceeding_type:, level_of_help:, submission_date:).tap do |e|
          e.assessment_result = assessed_result(assessed_capital:, lower_threshold: e.lower_threshold, upper_threshold: e.upper_threshold)
        end
      end

      def create_unassessed(proceeding_type:, level_of_help:, submission_date:)
        create_eligibility(proceeding_type:, level_of_help:, submission_date:).tap do |e|
